...
public class DriveStickSlew extends CommandBase {
    ...
    public void execute() {
        ...
        final double xSpeed = -xSpeedLimiter.calculate(drive.deadbandStick(controller.getY(GenericHID.Hand.kLeft)))
                * frc.robot.Drivetrain.kMaxSpeed;

        // Get the y speed or sideways/strafe speed. We are inverting this because
        // we want a positive value when we pull to the left. Xbox controllers
        // return positive values when you pull to the right by default.
        final double ySpeed = -ySpeedLimiter.calculate(drive.deadbandStick(controller.getX(GenericHID.Hand.kLeft)))
                * frc.robot.Drivetrain.kMaxSpeed;
        ...
        final double rot = -rotLimiter.calculate(drive.deadbandStick(controller.getX(GenericHID.Hand.kRight)))
